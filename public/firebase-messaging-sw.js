...
messaging.onBackgroundMessage((payload) => {
  ...
  if (
    notificationType === "user-declined" ||
    notificationType === "user-deleted"
  ) {
  ...
  if (
    notificationType === "user-verified" ||
    notificationType === "user-role-updated"
  ) {
    self.clients.matchAll({ type: "window" }).then((clients) => {
      clients.forEach((client) => {
        client.postMessage({
  ...
  // If the payload has a 'notification' property, the browser/OS automatically shows it.
  ...
  const notificationTitle = payload.data?.title || "Techser Sales";

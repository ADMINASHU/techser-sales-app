...
import { Mail, Phone, MapPin, Building2, Globe, ShieldCheck, Activity } from "lucide-react"; // [NEW]
...
export default async function ProfilePage() {
    ...
    const [user, totalEntries, lastEntry] = await Promise.all([
        ...
        Entry.findOne({ userId: session.user.id }).sort({ createdAt: -1 }).select("createdAt")
    ]);
            ...
            <div className="relative overflow-hidden rounded-2xl glass-card p-5 md:p-8">
                ...
                <div className="relative z-10 flex flex-row items-center gap-4 md:gap-8">
                    ...
                    <div className="relative shrink-0">
                        ...
                        <div className={`absolute bottom-2 right-2 h-3.5 w-3.5 md:h-5 md:w-5 rounded-full border-4 border-[#030712] ${user.status === 'verified' ? 'bg-emerald-500' : 'bg-yellow-500'}`} title={user.status} />
                    </div>

                    {/* Info */}
                    <div className="flex-1 min-w-0 space-y-2 md:space-y-3">
                        <div>
                            <h1 className="text-2xl md:text-4xl font-bold text-white tracking-tight truncate">{user.name}</h1>
                        ...
                        <div className="flex flex-col md:flex-row md:items-center gap-2 w-full">
                            ...
                            {user.region && (
                                <div className="flex items-center text-xs font-medium text-gray-300 bg-white/5 px-3 py-0.5 rounded-full border border-white/10 w-fit max-w-full">
                                    <svg className="w-3 h-3 mr-1.5 text-cyan-400 shrink-0" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M17.657 16.657L13.414 20.9a1.998 1.998 0 01-2.827 0l-4.244-4.243a8 8 0 1111.314 0z" /><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M15 11a3 3 0 11-6 0 3 3 0 016 0z" /></svg>
                                    <span className="truncate">{user.branch}, {user.region}</span>
                                </div>
                            )}
                        </div>
                    </div>
                    ...
                    <div className="shrink-0 absolute top-0 right-0 md:static md:top-auto md:right-auto">
                        <EditProfileDialog user={userPlain} trigger={
                            <Button className="glass-btn-primary h-9 w-9 p-0 md:h-10 md:w-auto md:px-6 rounded-full md:rounded-lg shadow-lg hover:shadow-xl hover:-translate-y-0.5 transition-all" title="Edit Profile">
                                <svg className="w-4 h-4 md:mr-2" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M15.232 5.232l3.536 3.536m-2.036-5.036a2.5 2.5 0 113.536 3.536L6.5 21.036H3v-3.572L16.732 3.732z" /></svg>
                                <span className="hidden md:inline text-sm font-medium">Edit Profile</span>
                            </Button>
                        } />
                    </div>
                </div>
                    ...
                    <div className="space-y-6">
                        <div className="grid grid-cols-1 sm:grid-cols-2 gap-6">
                            <div className="group">
                                ...
                                <div className="text-lg text-gray-200 group-hover:text-white transition-colors truncate" title={user.email}>{user.email}</div>
                            </div>
                            <div className="group">
                                <label className="text-xs uppercase tracking-wider text-gray-500 font-semibold mb-2 flex items-center gap-2">
                                ...
                                <div className="text-lg text-gray-200 group-hover:text-white transition-colors">{user.contactNumber || "Not Provided"}</div>
                            </div>
                        </div>
                        <div className="group">
                            <label className="text-xs uppercase tracking-wider text-gray-500 font-semibold mb-2 flex items-center gap-2">
                            ...
                            <div className="text-lg text-gray-200 group-hover:text-white transition-colors leading-relaxed">{user.address || "Not Provided"}</div>
                        </div>
                    </div>
                </div>
                    ...
                    <div className="space-y-6">
                        <div className="grid grid-cols-1 sm:grid-cols-2 gap-6">
                            <div className="group">
                                ...
                                <div className="text-lg text-white font-medium">{user.branch || "N/A"}</div>
                            </div>
                            <div className="group">
                                <label className="text-xs uppercase tracking-wider text-gray-500 font-semibold mb-2 flex items-center gap-2">
                                ...
                                <div className="text-lg text-white font-medium">{user.region || "N/A"}</div>
                            </div>
                        </div>
                        ...
                        <div className="pt-4 border-t border-white/5">
                            <div className="grid grid-cols-1 sm:grid-cols-2 gap-6">
                                <div>
                                    ...
                                    <Badge variant="outline" className={`uppercase tracking-wider px-2 py-0.5 w-fit text-[10px] md:text-xs ${user.role === 'admin' ? 'bg-violet-500/20 text-violet-300 border-violet-500/30' : 'bg-blue-500/20 text-blue-300 border-blue-500/30'}`}>
                                ...
                                <div>
                                    ...
                                    <div className={`text-sm font-bold uppercase tracking-wide ${user.status === 'verified' ? 'text-emerald-400' : 'text-yellow-400'}`}>
                                        {user.status}
                                    </div>
                                </div>
                            </div>

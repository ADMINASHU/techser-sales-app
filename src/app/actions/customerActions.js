...
export async function updateCustomer(id, formData) {
  ...
  try {
    ...
    const isAuthorized =
      ...
      (session.user.region === customer.region &&
        session.user.branch === customer.branch);
...
export async function toggleCustomerShare(id) {
  const session = await auth();
  if (!session) return { error: "Not authenticated" };
...
export async function getCustomers({
  ...
  try {
    ...
    // For non-admins, filter by ownership OR shared status within same branch/region
    if (!isAdmin) {
    ...
    if (filters.search) {
      ...
      if (query.$or) {
        query.$and = [
          { $or: query.$or },
          { $or: [{ name: searchRegex }, { customerAddress: searchRegex }] },
        ];
        delete query.$or;
      } else {
        query.$or = [{ name: searchRegex }, { customerAddress: searchRegex }];
      }
    }
...
export async function getCustomersWithEntryCount({
  filters = {},
  skip = 0,
  limit = 15,
  activeOnly = false,

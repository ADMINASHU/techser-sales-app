...
export default function CustomerActionCard({ customer, activeEntry, userId, hasActiveStampIn }) {
    ...
    return (
        <div className={`glass-card p-4 rounded-xl relative overflow-hidden group transition-all duration-300 transform-gpu ${
            isStampedIn ? "border-yellow-500/30 ring-1 ring-yellow-500/10 shadow-[0_0_25px_rgba(234,179,8,0.1)]" : ""
        }`}>
            {/* Background Glow */}
            ...
            <div className={`absolute left-0 top-0 bottom-0 w-1.5 z-20 ${
                isStampedIn ? "bg-yellow-500 shadow-[0_0_10px_rgba(234,179,8,0.5)]" : 
            ...
            <div className="flex justify-between items-start mb-2 gap-4 relative z-10">
                ...
                <div className="flex flex-col items-end gap-1.5 shrink-0">
                    <Badge variant="outline" className={`text-[9px] py-0 px-1.5 font-medium h-4 ${
                        customer.entryCount > 0 ? "bg-white/5 text-emerald-400 border-white/10" : "bg-white/5 text-gray-500 border-white/10"
                    }`}>

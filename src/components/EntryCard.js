...
const EntryCard = memo(function EntryCard({ entry, isAdmin, onDelete }) {
  ...
  const isUnverified =
    (inDistance !== null && inDistance > 100) ||
    (outDistance !== null && outDistance > 100);
    ...
    <>
      ...
      >
        ...
        {isAdmin && (
          <div className="mb-3 pb-3 border-b border-white/5 flex items-center justify-between gap-2 relative z-10">
            ...
            <p className="text-xs text-gray-500 whitespace-nowrap">
              {entry.userId?.branch || entry.branch} /{" "}
              {entry.userId?.region || entry.region}
            </p>
          </div>
        )}
        ...
        <div className="relative mb-3 flex-1 z-10">
          ...
          {isAdmin &&
            isUnverified &&
            (inDistance !== null || outDistance !== null) && (
              <div className="mt-1 flex gap-3 text-xs font-mono">
                {inDistance !== null && (
                  <span
                    className={`flex items-center gap-1 ${inDistance > 100 ? "text-red-400 font-bold" : "text-emerald-400"}`}
                  >
                ...
                {outDistance !== null && (
                  <span
                    className={`flex items-center gap-1 ${outDistance > 100 ? "text-red-400 font-bold" : "text-emerald-400"}`}
                  >
          ...
          <div className="flex items-center justify-between text-xs text-gray-500 border-t border-white/5 pt-3">
            ...
            <div className="flex items-center gap-2">
              ...
              {(entry.status === "In Process" ||
                entry.status === "Completed") && (
                <div
                  className={`px-2.5 py-1 rounded-full text-xs font-medium border flex items-center gap-1.5 shadow-sm ${
                    entry.status === "Completed"
                      ? "bg-emerald-500/10 text-emerald-400 border-emerald-500/20"
                      : "bg-yellow-500/10 text-yellow-400 border-yellow-500/20"
                  }`}

...
export default async function EntryListContainer({ searchParams, session }) {
  ...
  if (!isCoreAdmin) {
    ...
    // For Super User, always restrict by region if they are viewing admin logs
    if (isSuperUser) {
      params.region = session.user.region;
    }
  }
  ...
  if (
    ...
    if (params.region && params.region !== "all")
      userQuery.region = params.region;
    if (params.branch && params.branch !== "all")
      userQuery.branch = params.branch;

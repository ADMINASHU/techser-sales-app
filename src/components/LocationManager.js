...
export default function LocationManager({ initialLocations }) {
  ...
  const selectedRegion =
    locations.find((l) => l._id === selectedRegionId) ||
      ...
      setLocations(
        locations.map((l) => {
          if (l._id === selectedRegion._id) {
            return {
              ...l,
              branches: l.branches.filter((b) => b !== branchName),
            };
          }
          return l;
        })
      ...
      <div className="glass-panel border-white/5 rounded-xl flex flex-col overflow-hidden shadow-2xl">
        ...
        <div className="flex-1 overflow-y-auto p-4 space-y-2">
          ...
          {locations.map((loc) => (
            <div
              key={loc._id}
              onClick={() => setSelectedRegionId(loc._id)}
            ...
            >
              <span
                className={`font-medium ${
                  selectedRegion?._id === loc._id ? "text-violet-200" : ""
                }`}
              >
                {loc.name}
              </span>
              <Button
                ...
                className={`h-8 w-8 p-0 lg:opacity-0 lg:group-hover:opacity-100 transition-opacity ${
                  selectedRegion?._id === loc._id
                    ? "text-violet-300 hover:text-red-400"
